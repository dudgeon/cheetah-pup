...
def kinematic_audit(config: dict, samples: int = 32, seed: int = 731):
    ...
    errors = {
        "fk_max_error_m": 0.0,
        "jacobian_mujoco_max_error_m_per_rad": 0.0,
    ...
    for _ in range(samples):
        ...
        for leg in LEG_ORDER:
            ...
            data.qpos[qadr] = [
                rng.uniform(*config["joint_limits_rad"][j]) for j in JOINT_ORDER
            ]
        mujoco.mj_forward(model, data)
        ...
        for leg in LEG_ORDER:
            ...
            errors["fk_max_error_m"] = max(
                errors["fk_max_error_m"],
                float(np.max(np.abs(predicted - data.site_xpos[site]))),
            )
            jp, jr = np.zeros((3, model.nv)), np.zeros((3, model.nv))
            mujoco.mj_jacSite(model, data, jp, jr, site)
            analytical = leg_jacobian(config, leg, q)
            errors["jacobian_mujoco_max_error_m_per_rad"] = max(
                errors["jacobian_mujoco_max_error_m_per_rad"],
                float(np.max(np.abs(rotation @ analytical - jp[:, dadr]))),
            )
            step = 1e-6
            finite = np.column_stack(
                [
                    (
                        foot_position(config, leg, q + np.eye(3)[j] * step)
                        - foot_position(config, leg, q - np.eye(3)[j] * step)
                    )
                    / (2 * step)
                    for j in range(3)
            ...
            errors["jacobian_finite_difference_max_error_m_per_rad"] = max(
                errors["jacobian_finite_difference_max_error_m_per_rad"],
                float(np.max(np.abs(analytical - finite))),
            )
    return {
        "samples": samples,
        "legs_per_sample": 4,
...
def static_support_screen(config: dict, model=None, data=None):
    ...
    cases += [
        (f"three_feet_lift_{leg}", tuple(l for l in LEG_ORDER if l != leg))
        for leg in LEG_ORDER
    ]
    ...
    for name, supports in cases:
        forces = solve_vertical_support(
            [foot[l][:2] for l in supports], com[:2], weight
        )
        base = {
            "case": name,
            "support_feet": list(supports),
            "vertical_static_equilibrium": forces is not None,
        }
        if forces is None:
            results.append(
                {
                    **base,
                    "reason": "The current COM projection is outside this support polygon/line; body shift or dynamic motion is required.",
                }
            )
            continue
        ...
        torques = {
            f"{leg}_{joint}": float(
                generalized[model.jnt_dofadr[model.joint(f"{leg}_{joint}").id]]
            )
            for leg in LEG_ORDER
        ...
        results.append(
            {
                **base,
                "vertical_foot_load_n": dict(zip(supports, map(float, forces))),
                "joint_torque_nm": torques,
                "peak_abs_joint_torque_nm": peak,
    ...
    return {
        "method": "Neutral-pose vertical force/moment equilibrium and qfrc_bias - J^T F; excludes dynamic gait loads, friction-model fidelity and thermals.",
        "com_world_m": com.tolist(),
        "foot_centers_world_m": {k: v.tolist() for k, v in foot.items()},
        "cases": results,
    }
...
def pd_sanity_rollout(config: dict, seconds: float = 5.0):
    ...
    return {
        "model_type": "Unidentified ideal position PD; no BAM and no policy",
        ...
        "peak_abs_actuator_torque_nm": peak_torque,
        "fraction_steps_any_actuator_at_limit": saturation_steps / max(steps, 1),
        "final_max_joint_speed_rad_s": float(np.max(np.abs(data.qvel[6:])))
        if finite
        else None,
...
def battery_screen():
    return {
        "status": "Sizing assumptions, no selected battery or measured endurance",
        "nominal_voltage_v": 7.4,
        "candidate_capacity_ah": 0.65,
        "nominal_energy_wh": 7.4 * 0.65,
        "usable_fraction": 0.8,
        "assumed_conversion_efficiency": 0.88,
        "allowable_average_output_power_w": {
            "10_minutes": 7.4 * 0.65 * 0.8 * 0.88 / (10 / 60),
            "15_minutes": 7.4 * 0.65 * 0.8 * 0.88 / (15 / 60),
        },
...
def sizing_sweep(config: dict):
    ...
    rows = []
    for nonmotor_scale, length_offset, hip_pitch in itertools.product(
        (0.8, 1.0, 1.2), (-0.01, 0, 0.01), (0.25, 0.35, 0.4, 0.5, 0.6)
    ):
        ...
        viable_three = [
            ...
            if case["case"].startswith("three_") and case["vertical_static_equilibrium"]
        ]
        rows.append(
            {
                "nonmotor_mass_scale": nonmotor_scale,
                "mass_kg": total_mass(candidate),
                "upper_length_m": candidate["geometry_m"]["upper_length"],
                "lower_length_m": candidate["geometry_m"]["lower_length"],
                "home_hip_pitch_rad": hip_pitch,
                "four_foot_peak_torque_nm": four["peak_abs_joint_torque_nm"],
                "four_foot_static_margin": four["static_margin_ratio"],
                "worst_statically_supported_three_foot_margin": min(
                    c["static_margin_ratio"] for c in viable_three
                )
                if viable_three
                else None,
                "three_foot_cases_statically_possible_without_body_shift": len(
                    viable_three
                ),
            }
        )
    return {
        ...
        "notes": "Parameter sensitivity only. Twelve servo masses and envelopes remain unchanged. Nonmotor allowances vary ±20%; link lengths vary ±10mm. No hardware packaging, singularity reserve, or complete gait validation. Three-foot rows use the unchanged body pose, not an optimized crawl.",
        "variants": rows,
    }
...
def make_report(config: dict):
    ...
    foot_soles = [
        float(data.site(f"{leg}_foot").xpos[2] - config["geometry_m"]["foot_radius"])
        for leg in LEG_ORDER
    ]
    return {
        "stage": "Refined assembly geometry with ideal-PD diagnostic screening; separate BAM and crawl reports available",
        ...
        "user_requirements": {
            "size": "smallest that keeps construction straightforward",
            "terrain": "carpet and small doorway thresholds",
            "active_walking_minutes": [10, 15],
            "owner_servo_characterization": False,
        },
        "structure": {
            "actuators": model.nu,
            "hinge_joints": model.njnt - 1,
            "free_joints": 1,
            "config_mass_kg": expected_mass,
            "mujoco_mass_kg": mass,
            "mass_error_kg": abs(expected_mass - mass),
            "foot_sole_height_m": foot_soles,
        ...
        "static_support": static_support_screen(config, model, data),
        "sizing_sweep": sizing_sweep(config),
        "pd_sanity": pd_sanity_rollout(config),
        "battery_screen": battery_screen(),
        "gates": {
            "geometry_implementation": "pass"
            if kinematics["passed"] and abs(expected_mass - mass) < 1e-9
            else "fail",
            "motor_selection": "open: conservative gait load, model provenance and budget must converge",
            "realistic_actuator_physics": "CPU published BAM adapter implemented separately; see actuator-validation.json; stock-5V calibration remains open",
            "manufacturing_and_self_collision": "sampled casing/cable clearance audited separately; see assembly-validation.json; detailed manufacturing design remains open",
            "rl_training": "not started",
            "carpet_and_threshold_traversal": "not demonstrated",
            "battery_runtime": "not demonstrated",
...
def write_report(config: dict, output: Path):
    ...
    lines = [
        ...
        "",
        f"Environment: MuJoCo {mujoco.__version__}, Python {platform.python_version()}.",
        "",
        ...
        "",
        f"The diagnostic model has **{structure['actuators']} joints** and an estimated **{structure['mujoco_mass_kg'] * 1000:.0f} g** component budget. The central chassis envelope is **{dims} mm**; that is not the full robot's exterior size.",
        "",
        "## Geometry checks",
        "",
    ...
    for case in result["static_support"]["cases"]:
        if case["vertical_static_equilibrium"]:
            lines.append(
                f"| {case['case']} | Yes | {case['peak_abs_joint_torque_nm']:.4f} N·m | {case['static_margin_ratio']:.2f}× |"
            )
        else:
