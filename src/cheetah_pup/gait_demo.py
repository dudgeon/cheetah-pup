...
from .render import _box


def inverse_leg(config, leg, target):
    g, signs = config["geometry_m"], config["legs"][leg]
    x, y, z = np.asarray(target) - [signs["front"] * g["hip_x"], signs["side"] * g["hip_y"], g["hip_z"]]
    ...
    cosine = (x*x + planar_z*planar_z - upper*upper - lower*lower) / (2 * upper * lower)
    if abs(cosine) > 1 + 1e-10:
        raise ValueError(f"Unreachable foot target for {leg}")
    knee = -np.arccos(np.clip(cosine, -1, 1))
    hip = np.arctan2(-x, -planar_z) - np.arctan2(lower * np.sin(knee), upper + lower * np.cos(knee))
...
def trajectory(config, frames_per_step=24, stride=.020, clearance=.012):
    model, data = standing_model(config)
    feet = {leg: data.site(f"{leg}_foot").xpos.copy() for leg in LEG_ORDER}
    base = data.qpos[:3].copy()
    base[2] = .124  # Demonstration stance; more knee clearance, not a torque optimization.
    ...
    for cycle in range(2):
        for step, moving in enumerate(sequence):
            ...
            for frame in range(frames_per_step):
                ...
                if phase < .375:
                    base_now = start_base + smooth(phase / .375) * (shifted - start_base)
                    active = LEG_ORDER
                ...
                for leg in LEG_ORDER:
                    max_error = max(max_error, float(np.max(np.abs(data.site(f"{leg}_foot").xpos - current_feet[leg]))))
                if len(active) == 3:
                ...
                if cycle == 1:
                    frames.append({"qpos": data.qpos.copy(), "active": active, "moving": moving,
                                   "label": label, "progress": (step + phase)/4,
                                   "view_offset": stride * (cycle + (step + phase)/4)})
            feet[moving][0] += stride
            base = shifted.copy()
    return model, frames, {"kind": "prescribed kinematic crawl; not learned or dynamically validated",
                           "frames": len(frames), "stride_per_cycle_m": stride,
                           "playback_fps": 15, "cycle_duration_s": len(frames) / 15,
                           "foot_clearance_m": clearance, "base_height_m": .124,
                           "max_ik_foot_error_m": max_error,
                           "minimum_swing_support_load_fraction": min_swing_margin,
                           "joint_limits_checked": True, "stance_feet_world_locked": True,
...
def round_faces(radius, half=0):
    ...
    for angle in np.linspace(-np.pi/2, np.pi/2, 8):
        ...
        rings.append([[radius*np.cos(angle)*np.cos(phi), radius*np.cos(angle)*np.sin(phi), z] for phi in np.linspace(0, 2*np.pi, 12, endpoint=False)])
    return [[rings[i][j], rings[i][(j+1)%12], rings[i+1][(j+1)%12], rings[i+1][j]] for i in range(7) for j in range(12)]
...
def render_frames(config, output):
    ...
    ax = fig.add_axes([.01, .14, .67, .70], projection="3d", computed_zorder=False)
    ax.set_facecolor("#f4f3ef")
    ax.set_axis_off()
    ax.set_xlim(-.14, .14); ax.set_ylim(-.14, .14); ax.set_zlim(0, .21)
    ax.set_box_aspect((.28, .28, .21), zoom=1.3)
    ax.set_proj_type("ortho"); ax.view_init(elev=23, azim=-46)
    robot = Poly3DCollection([], facecolors=colors, edgecolors=(.08, .13, .16, .15), linewidths=.2, zorder=3)
    ax.add_collection3d(robot)
    floor = Poly3DCollection([[[-.16,-.14,0],[.16,-.14,0],[.16,.14,0],[-.16,.14,0]]], facecolors="#e1e5e0", edgecolors="#c2cbc3", linewidths=.5, zorder=0)
    ax.add_collection3d(floor)
    top = fig.add_axes([.72, .26, .24, .43])
    fig.text(.06,.91,"CHEETAH PUP · SLOW CRAWL", fontsize=19, weight="bold", color="#173447")
    phase_text = fig.text(.06,.85,"",fontsize=12,color="#426071")
    fig.text(.06,.08,"Planned gait · inverse kinematics · 12 mm foot lift", fontsize=11,color="#426071")
    fig.text(.06,.035,"Illustration only: not a trained policy or validated motor/contact simulation.",fontsize=10,color="#69777b")
    fig.text(.73,.18,"● Planted",fontsize=9,color="#398360")
    fig.text(.85,.18,"● Swing",fontsize=9,color="#d58b23")
    fig.text(.76,.145,"× Center of mass",fontsize=9,color="#bd5547")
    for n, frame in enumerate(frames):
        ...
        for index, faces in zip(indices, local_faces):
            verts.extend(faces @ data.geom_xmat[index].reshape(3,3).T + data.geom_xpos[index] - offset)
        robot.set_verts(verts)
        phase_text.set_text(frame["label"])
        top.clear(); top.set_facecolor("#f4f3ef")
        ...
        ordered=active[np.argsort(np.arctan2(active[:,1]-center[1],active[:,0]-center[0]))]
        top.fill(ordered[:,1]*1000, ordered[:,0]*1000, color="#dce9df", ec="#729781",lw=1.2)
        for leg, xyz in pts.items():
            planted=leg in frame["active"]
            top.scatter(xyz[1]*1000,xyz[0]*1000,s=70,c="#398360" if planted else "#e69a32",zorder=4)
            top.text(xyz[1]*1000+6,xyz[0]*1000+5,leg,fontsize=9,color="#334c58")
        com=data.subtree_com[model.body("base").id]-offset
        top.scatter(com[1]*1000,com[0]*1000,s=60,c="#bd5547",marker="x",zorder=5,linewidths=2)
        top.set_xlim(95,-95); top.set_ylim(-100,100); top.set_aspect("equal"); top.set_axis_off()
        top.set_title("Support from above\n↑ Forward",fontsize=11,color="#334c58",pad=9)
        fig.savefig(output/f"{n:04d}.png",dpi=100,facecolor=fig.get_facecolor())
        if n % 24 == 0:
            print(f"Rendered {n}/{len(frames)}",flush=True)
    plt.close(fig)
    Path("reports/gait-demo-validation.json").write_text(json.dumps(report,indent=2)+"\n")
