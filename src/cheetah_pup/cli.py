...
def main():
    ...
    build.add_argument(
        "--terrain", choices=("flat", "threshold", "carpet"), default="flat"
    )
    build.add_argument(
        "--output", type=Path, default=Path("models/cheetah_pup_flat.xml")
    )
    validate = commands.add_parser(
        "validate", help="Check geometry and record load screening"
    )
    validate.add_argument(
        "--output", type=Path, default=Path("reports/primitive-validation.json")
    )
    render = commands.add_parser(
        "render", help="Render the initial pose; no gait or policy"
    )
    render.add_argument(
        "--output", type=Path, default=Path("reports/primitive-preview.png")
    )
    render.add_argument(
        "--renderer", choices=("software", "mujoco"), default="software"
    )
    ...
    if args.command == "build":
        ...
        print(
            f"Geometry: {report['gates']['geometry_implementation']}; mass: {report['structure']['mujoco_mass_kg']:.3f} kg"
        )
        print(f"Wrote {args.output} and {args.output.with_suffix('.md')}")
        print(
            "Motor selection, BAM calibration, RL and hardware gates remain open; see separate BAM report."
        )
        ...
        if args.renderer == "software":
            ...
            print(
                f"Wrote {args.output}; software projection of the neutral MuJoCo model"
            )
            return
        import mujoco
        import numpy as np
