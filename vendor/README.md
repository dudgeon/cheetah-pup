...
| `open_duck_playground` | [apirrone/Open_Duck_Playground](https://github.com/apirrone/Open_Duck_Playground) | **no LICENSE file, but** the env/randomize/joystick/constants files carry **Apache-2.0 headers** (DeepMind + Pirrone/Nguyen, derived from mujoco_playground); 18 of 23 files have no header | The RL training environment (MuJoCo Playground-based, JAX/PPO) used to train Open Duck Mini's policies. Reference for env structure, domain randomization, ONNX export pipeline. | The header-bearing files are usable under Apache-2.0 with attribution; treat the rest as reference-only pending clarification. Our `cheetah_pup/rl/` is written against mujoco_playground's own Apache-2.0 quadruped envs instead. |
